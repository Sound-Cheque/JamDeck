...
- [~] Canvas data model: `objects[]` with per-kind handler registry for `bbox` + `hitTest`. v1 registers `stroke` only (rect/circle/line/arrow/triangle/text follow in subsequent commits). Pure functions: `newCanvas`, `addObject`, `removeObject`, `updateObject`, `getObject`, `bbox`, `hitTest`, plus `createStroke`. JSON round-trip; topmost-wins for overlapping hit-tests. 18 tests.
- [~] Canvas editor UI — `CanvasEditor` with freehand brush (TDD'd), `SlideEditor` dispatcher (canvas / image / unknown). The in-progress stroke lives in a ref (no React re-render per pixel); commit happens on mouseup with `addObject` → `updateSlide` → server PATCH. Switched from pointer events to mouse events (jsdom's PointerEvent doesn't propagate `clientX/Y` or `button` through fireEvent — touch/pointer support deferred to Phase 5 mobile work). Shape palette and text tool still pending.
